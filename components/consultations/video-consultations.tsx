...
export function VideoConsultations() {
  ...
  const handleBookSession = (astrologer: (typeof astrologers)[0]) => {
    ...
    <>
      ...
      {selectedAstrologer && (
        <BookingModal
          ...
          serviceName={`${selectedAstrologer.duration}-minute Video Consultation with ${selectedAstrologer.name}`}
          servicePrice={`₹${selectedAstrologer.price.toLocaleString("en-IN")}`}
        />
      )}
    </>
  )
